...
def dig(domain,timestart,ip = "198.41.0.4"): #ip is defaulted to a root server IP, can be changed if necessary
    ...
    if (ans == []): 
        ...
        if not add == []:   #
            ...
            for x in add[index]:        #recursive call with a new IP
                return dig(domain, timestart, str(x))
        elif not auth == []:
            spl = str(auth[0]).split()  #same as additional
            if spl[3] == "AAAA":
                index = 1
            for x in auth[index]:
                return dig(str(x), timestart) #recursive call with new domain and default IP
        else:
...
if __name__ ==  "__main__":
    ...
    ansSection = dig(domain, start)   #query
